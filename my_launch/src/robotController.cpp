...
class reCon_controllerNode : public rclcpp::Node{
public:
  reCon_controllerNode() : Node("reCon_controllerNode"){
    // publish for /feedback
    publisher_fb = this->create_publisher<std_msgs::msg::String>("/feedback", 10);
    // subscriber for /cmdToControl
    subscriber_cmd = this->create_subscription<std_msgs::msg::Int32>(
      "cmdToControl", 10,
      std::bind(&reCon_controllerNode::handleCmdToControl, this, std::placeholders::_1));
  }
  ...
  void handleCmdToControl(const std_msgs::msg::Int32::SharedPtr msg){
    ...
    if(msg->data == 2){
      ...
      if (kill(main_pid, SIGSTOP) == -1) {
          perror("pause");
      }
      std::cout<<"CONTINUE FOLOWING THE PATH. "<<std::endl;
      feedback_msg.data = "Stop following the path";
    }else{
      if(kill(main_pid,SIGCONT) == -1) {
        perror("continue");
      }
      std::cout<<"CONTINUE FOLOWING THE PATH. "<<std::endl;
      feedback_msg.data = "continue following the path";
    }
...
int main(int argc, char *argv[])
{
  ...
  if(child_pid == 0){
    ...
    */
    auto node = std::make_shared<reCon_controllerNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
  }else{
